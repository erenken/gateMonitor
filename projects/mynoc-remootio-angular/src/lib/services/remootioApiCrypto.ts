 ...
 *
 * Converted from remootio node library (https://github.com/remootio/remootio-api-client-node/blob/master/src/apicrypto.ts)
 */

import { AES, lib, enc, HmacSHA256, mode, pad } from 'crypto-js';
import { EncryptedFrame, ReceivedEncryptedFrameContent } from './remootioFrames';
...
export function remootioApiDecryptEncrypedFrame(
  ...
  if (ApiSessionKey == undefined) {
    ...
    CurrentlyUsedSecretKeyWordArray = enc.Hex.parse(ApiSecretKey); //Parse hexstring
  } else {
    ...
    CurrentlyUsedSecretKeyWordArray = enc.Base64.parse(ApiSessionKey); //Parse hexstring
  }
  ...
  const ApiAuthKeyWordArray = enc.Hex.parse(ApiAuthKey); //Parse hexstring

  //Step 1 verify MAC
  //It is a HMAC-SHA256 over the JSON.stringify(frame.data)
  const mac = HmacSHA256(JSON.stringify(frame.data), ApiAuthKeyWordArray);
  const base64mac = enc.Base64.stringify(mac);
  ...
  const payloadWordArray = enc.Base64.parse(frame.data.payload);
  //The frame.data.iv is a base64 encoded string, we convert it to wordArray for CryptoJS
  const ivWordArray = enc.Base64.parse(frame.data.iv);

  const decryptedPayloadWordArray = AES.decrypt(
    lib.CipherParams.create({ ciphertext: payloadWordArray }),
  ...
  const decryptedPayload = enc.Latin1.stringify(decryptedPayloadWordArray); //The decrypted data is Latin1 encoded string representing a stringified JSON object
...
export function remootioApiConstructEncrypedFrame(
  ...
  const ivWordArray = lib.WordArray.random(16);
  //Convert the unencrypted payload to wordArray for CryptoJS
  const unencryptedPayloadWordArray = enc.Latin1.parse(unencryptedPayload);
  //Do the encryption
  const encryptedData = AES.encrypt(unencryptedPayloadWordArray, CurrentlyUsedSecretKeyWordArray, {
    iv: ivWordArray,
    mode: mode.CBC,
    padding: pad.Pkcs7
  });
  ...
  const toHMACObj = {
    iv: enc.Base64.stringify(ivWordArray), //IV is a base64 encoded string
    payload: enc.Base64.stringify(encryptedPayloadWordArray)
  };
  ...
  const mac = HmacSHA256(toHMAC, ApiAuthKeyWordArray);
  const base64mac = enc.Base64.stringify(mac); //We convert the mac to a base64 array
